...
架构：3 序列 (Sx/Sy/Sx+y) × 3 模态 (ID/Image/Text) = 9 个 DomainTransformer
     预测层晚融合 + 跨域聚合

消融模式 (ablation 参数):
  - "id_only"      仅 ID embedding，单域 (Sx+Sy)，2 个 Transformer
  - "id_text"      ID + CLIP Text，单域，4 个 Transformer
  - "id_text_img"  ID + CLIP Text + CLIP Image，单域，6 个 Transformer
...
class CDSRModel(nn.Module):
    def __init__(self, n_items, id_feats, img_feats, tex_feats,
        ...
        # ---- Embedding 表（按需创建）----
        self.id_emb = nn.Embedding(n_items, input_dim)
        self.id_emb.weight.data.copy_(id_feats)
        self.id_proj = nn.Linear(input_dim, d_model)

        if "tex" in self.enabled_modalities:
            self.tex_emb = nn.Embedding(n_items, input_dim)
            self.tex_emb.weight.data.copy_(tex_feats)
            self.tex_proj = nn.Linear(input_dim, d_model)

        if "img" in self.enabled_modalities:
            self.img_emb = nn.Embedding(n_items, input_dim)
            self.img_emb.weight.data.copy_(img_feats)
            self.img_proj = nn.Linear(input_dim, d_model)

        # ---- 域掩码 ----
        self.register_buffer("movie_mask", movie_mask)
        self.register_buffer("book_mask", book_mask)

        # ---- DomainTransformers（按需创建）----
        ...
        for seq in sequences:
            for mod in self.enabled_modalities:
                setattr(self, f"tf_{seq}_{mod}", DomainTransformer(input_dim, **dt_kwargs))
    ...
    def _fuse(self, h_dict, domain_mask, all_dict):
        """余弦相似度 + 模态加权融合 → 域掩码（掩码在融合后，避免 0*(-inf)=NaN）"""
        def cosine(a, b):
